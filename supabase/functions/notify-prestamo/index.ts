...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { email, persona, numeroInventario, tipo, marca, modelo, fechaDevolucion } = await req.json()
    if (!email) {
    ...
    await sendMail({
      to: email,
      subject: 'Préstamo de equipo registrado',
      html: prestamoTemplate({ persona, numeroInventario, tipo, marca, modelo, fechaDevolucion }),
    })
