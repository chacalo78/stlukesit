...
function ModalResetPassword({ nombre, email, onClose, onSuccess }) {
  ...
  async function handleGuardar() {
    ...
    const { data, error } = await supabase.functions.invoke('reset-password', {
      body: { targetEmail: email, newPassword: password }
    })
    setLoading(false)
    if (error || data?.error) {
        ...
        {listo ? (
          <div style={{ padding: '24px' }}>
            <div style={{
              ...
              Contraseña de {nombre || email} actualizada correctamente.
            </div>
            <button
              onClick={onClose}
          ...
          <>
            ...
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div style={{ fontSize: '12px', color: '#9ab89c' }}>
                Vas a establecer una contraseña nueva para <strong style={{ color: '#e8f0e8' }}>{nombre || email}</strong> ({email}), sin necesidad de la contraseña anterior.
              </div>
              <div>
